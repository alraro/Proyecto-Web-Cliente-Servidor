...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // IDs de recién creados (pendiente validación)
    ...
    function renderTable(volunteers) {
        ...
        if (!volunteers.length) {
            tbody.innerHTML = "<tr><td colspan='5' class='table-empty'>No hay colaboradores registrados.</td></tr>";
            return;
        }
        volunteers.forEach(v => {
            ...
            tr.innerHTML = `
                ...
                <td>${isPending ? '<span class="badge-soon">Pendiente validación</span>' : '<span style="color:#2e7d32;font-weight:600">Activo</span>'}</td>
    ...
    tbody.addEventListener('click', (e) => {
        ...
        if (!btn) return;
        const v = JSON.parse(btn.dataset.edit);
        openForm(false, v);
    });
    ...
    btnSave.addEventListener('click', async () => {
        ...
        const body = { name, phone: phone || null, email: email || null, address: address || null };
        ...
        try {
            ...
            // Refrescar lista si hay campaña seleccionada
